...
twitter_cache = '206project2_caching.json'

try: 
	cache_file = open(twitter_cache, 'r')
	cache_contents = cache_file.read()
	twitter_dic = json.loads(cache_contents)
	cache_file.close()
except: 
	CACHE_DICTION = {}	
...
def get_umsi_data(): 
	...
	if data in CACHE_DICTION: 
		...
		for x in range(0,12): #list from 0-11 represents the pages added on to the end of the base url string 
			page_num = base_url + str(x) 
			access_page = requests.get(page_num, headers ={'User-Agent': 'SI_CLASS'}) 
			html_lst.append(access_page)
