...
File_name = '206project2_caching.json'

try:

	cache_file = open(File_name, 'r')
	twitter_cache_contents = cache_file.read()
...
for x in html_lst:
	...
	for y in people:
		umsi_names = y.find("div",{"property":"dc:title"}).text
		umsi_descriptions = y.find("div", {"class":"field-name-field-person-titles"}).text
		umsi_titles[umsi_names] = umsi_descriptions
...
def get_five_tweets(input_str): 
	tweets_lst = []

	key = "twitter_University of Michigan"
	if key not in CACHE_DICTION: 
		CACHE_DICTION[key] = {}

	if input_str in CACHE_DICTION[key]:
		value = CACHE_DICTION[key][input_str]

	else: 
		value = api.search(input_str)	
		CACHE_DICTION[key][input_str] = value 
		cache_file = open(File_name, 'w')
		cache_file.write(json.dumps(CACHE_DICTION))
		cache_file.close()

	for x in value['statuses'][0:5]: 
		tweets_lst.append(x['text'])
